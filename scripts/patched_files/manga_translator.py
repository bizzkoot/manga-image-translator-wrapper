...
class MangaTranslator:
    ...
    async def translate(self, image: Image.Image, config: Config, image_name: str = None, skip_context_save: bool = False) -> Context:
        ...
        # Set current_input_basename if image_name provided (helps external-trans-dir)
        try:
            if image_name:
                self.current_input_basename = os.path.splitext(os.path.basename(image_name))[0]
        except Exception:
            pass
    ...
    async def _run_text_translation(self, config: Config, ctx: Context):
        ...
        # 优先使用外部逐图翻译（若提供），无条件覆盖并跳过机器翻译
        # Always prefer external per-image translations when provided, skip MT entirely
        ...
        # Set translator to none to prevent token waste if prep_manual is True
        if self.prep_manual:
        ...
        if self.load_text:  
            ...
            # Keep current_input_basename in sync when loading text
            try:
                self.current_input_basename = input_filename
            except Exception:
                pass
            ...
            if self.save_text:  
                input_filename = os.path.splitext(os.path.basename(self.input_files[0]))[0]
                # Keep current_input_basename in sync when saving text
                try:
                    self.current_input_basename = input_filename
                except Exception:
                    pass
